...
const initInfo: SubscribeInfo = {
    list: [
        {key:'notPunchMessage', name: '未打卡提醒', loading: true, checked: true},
        {key:'schedulingMessage', name: '派勤提醒', loading: true, checked: true},
        {key:'attendanceMessage', name: '出勤提醒', loading: true, checked: true},
        {key:'clockOutMessage', name: '下班关闭打卡提醒', loading: true, checked: true},
        {key:'schedulingReminderMessage', name: '排班提醒', loading: true, checked: true},
        {key:'taskSignInMessage', name: '任务签收提醒', loading: true, checked: true},
        {key:'taskSchedulingMessage', name: '任务排班提醒', loading: true, checked: true},
        {key:'policeForceManage', name: '警力安排', loading: true, checked: true},
    ],
    isLoading: false,
};
...
export const getSubscribeInfo = createAsyncThunk('subscribeInfo/fetch', async (p, { getState, dispatch }) => {
    ...
    const temp = [rsp.notPunchMessage === 1, rsp.schedulingMessage === 1,
        rsp.attendanceMessage === 1,
        rsp.clockOutMessage === 1,
        rsp.schedulingReminderMessage === 1,
        rsp.taskSignInMessage === 1,
        rsp.taskSchedulingMessage === 1,
        rsp.policeForceManage === 1,
    ];
    return initInfo.list.map((i, idx) => ({
        ...i,
        checked: temp[idx],
        loading: false
