...
function DynamicTabs(props: {data: DynamicTabsInterface, originData: DynamicTabsInterface}) {
    ...
    return (
        <Tabs activeKey={activeKey} onChange={e => {
            ...
            {
                tabs.map((i, idx) => {
                    return  <Tabs.Tab title={<BarTitle title={i[0]} hasSplit={idx < tabs.length - 1} />} key={i[1]} destroyOnClose={true}>
                    <BarContent>
                        {
                            data[i[1]].length === 0 && 
                            <Empty image={<Image src='/imgs/statistics/empty.png' width={65} height={74} alt='' />} description='暂无结果~' />
                        }
                        {
                            data[i[1]].map(item => {
...
export default function ChildOverview(props: {currentOrg: CurrentOrgInterface}) {
    ...
    const [downDataList, setDownDataList] = useSafeState<DynamicTabsInterface>({4:[], 2: [], 3: []});
    ...
    useEffect(() => {
        Promise.all([
            axios.get(apis.getDownOrgsInfo, { /* 下属派出所 */
                params: {
            ...
            axios.get(apis.getDownOrgsInfo, { /* 下属机关单位 */
                params: {
                    orgCode: currentOrg.orgCode,
                    jgsx: 2,
                    postTypeId: ''
            ...
            axios.get(apis.getDownOrgsInfo, { /* 下属业务部门 */
                params: {
                    orgCode: currentOrg.orgCode,
                    jgsx: 3,
                    postTypeId: ''
