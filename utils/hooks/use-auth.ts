...
export function useAuth() {
    ...
    const setCode = useCallback((url?: string) => {
      const configCode = {
        base: false,
      ...
      if (code) {
        ...
        if (process.env.NEXT_PUBLIC_PROD === 'false') {
          ...
          location.href = "https://jwwx.gaj.sh.gov.cn/connect/oauth2/authorize?appid=" + configCode.appid + "&redirect_uri=" + encodeURIComponent(configCode.redirect_uri) + "&response_type=code&scope=" + scope + "&agentid=" + configCode.agentid + "&state=" + (configCode.base ? 3 : 2) + "#wechat_redirect"
        }
      }
    }
      , [searchParams, router]);
    const initUserInfo = useCallback((callback?: any) => {
      if (process.env.NEXT_PUBLIC_PROD === 'false') {
        ...
        axios.get(apis.getToken).then((res1: any) => {
          ...
          axios.get(`${proxy_url}/cgi-bin/user/getuserinfo`, {
            params: {
              access_token: res1,
              code
            }
            ...
            if (
              ...
              if (localStorage.getItem('UserId') !== res2.data.UserId) { /* 切换了用户 */
