...
function MsgItem(props: { item: any }) {
  ...
  if (!msgObj[props.item.messageType].title) return <></>;
  return (
    <div>
      <div className="mx-auto mb-[6px] h-5 w-fit rounded-[5px] bg-white px-[8px] text-center text-xs">{props.item?.pushTime}</div>
      <div className="mb-[30px] flex">
        <Image className="mr-[9px] h-[56px]" width={56} height={56} src={'/imgs/messages/icon-消息提醒@2x.png'} alt="" />
        <div className="mt-[7px] flex-1">
          <div className="mb-[7px] text-sm text-mitBlack">{msgObj[props.item.messageType].title}</div>
          <div className=" rounded-[20px] bg-white pb-[15px] pl-[21px] pr-[6px] pt-[15px] leading-5 text-mitBlack shadow-[0px_2px_7px_1px_rgba(61,110,232,0.1)]">
            {msgObj[props.item.messageType].info}
            {props.item.messageType === 5 &&
              (props.item?.schedulingType || []).map((i: 1 | 2 | 3) => {
                return (
                  <div key={i}>
                    {props.item?.orgName}明日【{t[i]}
                    】还未完成，请及时至勤务PC端排班！
                  </div>
                );
              })}
            {props.item.messageType === 6 && <>{props.item?.orgName}还有任务未签收，请及时签收！</>}
            {props.item.messageType === 7 && (
              <>
                {props.item?.orgName}
                还有任务相关岗位未排班，请及时至勤务PC端排班！
              </>
            )}
          </div>
        </div>
...
export default function Messages() {
  ...
  return (
    <div className="h-full overflow-y-auto" style={{ padding: msgs.length === 0 ? 0 : '32px 60px 0 13px' }} ref={listRef}>
      {msgs.map((item: any) => {
